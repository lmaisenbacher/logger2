...
- `HighFinesse`: HighFinesse wavemeters (tested with models WS Ultimate 2 MC and WS/7) (through Windows DLL API communicating with wavemeter software). Channel types: `Frequency` (THz, or GHz with `"Unit":"GHz"`); `AmplitudeMax1`/`AmplitudeMax2`/`AmplitudeAvg1`/`AmplitudeAvg2` (the interference pattern's maximum and average fringe height on CCD array 1 and 2, in counts; written with every result, valid or not); `Power` (the wavemeter's own reading of the light it received - pulse energy in uJ in a pulsed mode, power in uW in CW; relative, never calibrated); `Temperature` and `Pressure` (the optical unit's, read at most every `MinInterval_s` seconds of the channel's `DeviceSpecificParams`, default 60). One DLL pass per poll whatever the channel count; the amplitudes and the power are read only on a poll with a new result. The wavemeter's result status rides along as the string field `status` on the same row (renamed with `"status-field-key"`, dropped with `"status-field-key": null`) - one row per wavemeter result: a valid result writes the frequency and `status="ok"`, an error writes only the status text (`overexposed`, `underexposed`, `no_signal`, `no_pulse`, ..., `unknown_error`; vocabulary in `amodevices.HighFinesseWS.STATUS_TEXT`), and "nothing new" (ReadOnce mode) writes nothing. Write the status on the `Frequency` channel only (`"status-field-key": null` on every other channel, required on `Temperature`/`Pressure`): a second `status` series under the same sensor makes the frequency series ambiguous for a consumer that pins it by sensor (the ion-detection GUI), and the module refuses that layout at startup.
- `rp-lockbox`: Custom Red Pitaya lockbox [rp-lockbox](https://github.com/lmaisenbacher/rp-lockbox) (through SCPI over TCP/IP). Channel types per fast analog channel (`DeviceChannel` 1 or 2): `FastAnalogIn`, `FastAnalogOut`, `OutputMin`, `OutputMax` (V), `GeneratorState` (1/0, the signal generator on that output, which adds to the PID output); per auxiliary analog input (`DeviceChannel` 0–3): `AuxAnalogIn` (V); per PID controller (`PID` "11" to "22"): `GlobalGain`, `PGain`, `IGain`, `IIGain`, `DGain`, `Setpoint` (V), `PIDEnabled` (1/0, the "PID + relock output enabled" switch), `HoldState`, `RelockState` (1/0), `RelockMin`, `RelockMax` (V), `RelockStepsize` (V/s), `RelockInput` (V on the auxiliary input the PID's relock feature monitors), and `LockStatus`: 1/0 on every poll plus, on a change and at logger start, the companion fields `lock_event` ("locked", or "unlocked: relock input 0.120 V below window 0.500-1.200 V") and `lock_event_code` (1/-1) — the fields the cavity pointing PID server writes. `LockStatus` needs the rp-lockbox SCPI server with the `LOCKED?` query (newer than release 1.2.0). With the lockbox monitor service of rp-lockbox 1.3.0 (the box counts lock drops at 1 kHz and measures the input noise) also, per PID: `LockDrops` — the drops since the previous poll (0 on most polls) with the companions `unlocks_total`, `unlocked_s` (time in drops since the previous poll), `longest_unlock_s` (the longest drop that ended since) and `servo_drops` (since the hold was last switched off); per fast analog channel: `FastAnalogInRMS` — the input's standard deviation over the monitor's last window (V; the rms error in lock) with the companions `decimation` and `window_s` — and `FastAnalogInMean` (V). These write nothing while the monitor service is not running (one warning per outage); the other channels are unaffected.
